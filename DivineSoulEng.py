...
sheet2_id = "1NSSwAgrB8ngFLV2Lghl8-6exEt6z_pwU-DvNS3IyFaY"  # Substitua pelo forms
...
for email in repeated_emails:
    ...
    if email not in processed_emails_all:
        ...
        has_spiritual_connection_guide = any(
            record["Prd ID"] in [547414, 565473, 578202] for record in records1
        )

        has_connection_guide_Divine_Reading = any(
            record["Add-on product IDs"] == "547418" for record in records1
        )

        has_connection_guide_Past_Life_Reading = any(
            record["Add-on product IDs"] == "547421" for record in records1
        )

        has_connection_both_guide = any(
            record["Add-on product IDs"] == "547418|547421" for record in records1
        )
