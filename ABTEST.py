...
creds1 = ServiceAccountCredentials.from_json_keyfile_name(
    "divinesoulexport-e8513080cffb.json", scope
)
creds2 = ServiceAccountCredentials.from_json_keyfile_name(
    "divinesoulforms-965054994b76.json", scope
)
client1 = gspread.authorize(creds1)
client2 = gspread.authorize(creds2)

# Acesso às planilhas usando IDs em vez de nomes
sheet1_id = "1Cqjazkio5TWJh90EQODu75rx0goj5r-RPzgZWvlgotw"  # Substitua pelo export
sheet2_id = "1aeUS_cD_yhnWwll1KZRJyX7TneTcSXNxbpwuub8W4Gk"  # Substitua pelo forms
...
def enviar_email_SpiritualConnectionGuide(
    destinatario, assunto, corpo_texto, imagem=None
):
    msg = MIMEMultipart()
    msg["From"] = smtp_user
    msg["To"] = destinatario
...
def send_openai_request(mensagem):
    ...
    for attempt in range(retry_attempts):
        ...
        if response_openai.status_code == 200:
            ...
            print(
                f"Falha ao tentar acessar o código do GPT: {response_openai.status_code}"
            )
...
for email in repeated_emails:
    ...
    if email not in processed_emails_all:
        ...
        records2 = [
            record
            for record in data2
            if record["Email to receive your drawing?"] == email
        ]

        # Verificar produtos possuídos pelo dono do e-mail
        has_divine_soul = any(record["Prd ID"] == 559669 for record in records1)

        has_spiritual_connection_guide = any(
            record["Prd ID"] in [559673, 565473, 559670] for record in records1
        )

        has_connection_guide_Divine_Reading = any(
            record["Add-on product IDs"] == "559681" for record in records1
        )

        has_connection_guide_Past_Life_Reading = any(
            record["Add-on product IDs"] == "559682" for record in records1
        )

        has_connection_both_guide = any(
            record["Add-on product IDs"] == "559681|559682" for record in records1
        )
        ...
        if has_divine_soul:
            ...
            for record2 in records2:
                ...
                if resposta_openai:
                    ...
                    response_leonardo = leonardo.post_generations(
                        prompt=prompt,
                        num_images=1,
                    ...
                    if response_leonardo.status_code == 200:
                        ...
                        if (
                            "sdGenerationJob" in response_leonardo_json
                            and "generationId"
                            in response_leonardo_json["sdGenerationJob"]
                        ):
                            generation_id = response_leonardo_json["sdGenerationJob"][
                                "generationId"
                            ]
                            ...
                            if response_imagem.status_code == 200:
                                ...
                                if (
                                    "generations_by_pk" in response_imagem_json
                                    and "generated_images"
                                    in response_imagem_json["generations_by_pk"]
                                ):
                                    generated_images = response_imagem_json[
                                        "generations_by_pk"
                                    ]["generated_images"]

                                    if generated_images:
                                        image_url = generated_images[0]["url"]
                                        response_imagem_download = requests.get(
                                            image_url
                                        )
                                        ...
                                        if response_imagem_download.status_code == 200:
                                            with open(
                                                ...
                                                file.write(
                                                    response_imagem_download.content
                                                )
                                                print(
                                                    "Imagem salva com sucesso como generated_image.png"
                                                )
                                            ...
                                            if sucesso_envio:
                                                print(
                                                    f"Divine soul enviado com sucesso para: {email}"
                                                )
                                            else:
                                                print(
                                                    f"Falha ao enviar e-mail para: {email}"
                                                )
                                        else:
                                            print(
                                                f"Falha ao baixar a imagem, código de status: {response_imagem_download.status_code}"
                                            )
                                    else:
                                        print("Lista de 'generated_images' está vazia.")
                                else:
                                    print(
                                        "Chaves 'generations_by_pk' ou 'generated_images' não encontradas na resposta JSON da imagem."
                                    )
                            else:
                                print(
                                    f"Falha ao obter a URL da imagem, código de status: {response_imagem.status_code}"
                                )
                                print("Erro na resposta:", response_imagem.text)
                        else:
                            print(
                                "Chave 'generationId' não encontrada na resposta JSON da Leonardo AI."
                            )
                    else:
                        print(
                            f"Falha na requisição ao Leonardo AI, código de status: {response_leonardo.status_code}"
                        )
        ...
        if has_spiritual_connection_guide:
            for record2 in records2:
                ...
                if resposta_openai:
                    ...
                    sucesso_envio = enviar_email_SpiritualConnectionGuide(
                        email, assunto_imagem, resposta_texto
                    )

                    if sucesso_envio:
                        print(
                            f"Spiritual Connection Guide enviado com sucesso para: {email}"
                        )
                    else:
        ...
        if has_connection_guide_Divine_Reading:
            for record2 in records2:
                ...
                if preferencia_genero.lower() == "i like women":
                    ...
                    assunto_imagem = (
                        f"*{first_name},  Click Here To See Your Divine Reading!*"
                    )
                else:
                    idade_ajustada = age - 8
                ...
                if response_openai.status_code == 200:
                    ...
                    with open(filename, "w") as file:
                        file.write(resposta_texto)
                    print(f"Response saved successfully to {filename}!")
                    sucesso_envio = enviar_email_Divine_Reading(
                        email, assunto_imagem, resposta_texto
                    )
                    ...
                    print(
                        f"Failed to get OpenAI response, status code: {response_openai.status_code}"
                    )
        ...
        if has_connection_guide_Past_Life_Reading:
            for record2 in records2:
                ...
                assunto_imagem = (
                    f"*{first_name},  Click Here To See Your Divine Past Life Reading*"
                )
                data_openai = {
                    "model": "gpt-3.5-turbo",
                    "messages": [{"role": "user", "content": prompt}],
                ...
                if response_openai.status_code == 200:
                    ...
                    with open(filename, "w") as file:
                        file.write(resposta_texto)
                    print(f"Response saved successfully to {filename}!")
                    sucesso_envio = enviar_email_Past_Life_Reading(
                        email, assunto_imagem, resposta_texto
                    )
